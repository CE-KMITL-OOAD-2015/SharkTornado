...
public class PlanModel {
    ...
    protected Plan[] getUserPlanByCloud(Account user, String cloudProv){
        List<Plan> plans = vmProvider.getInstance().getPlanByCloudAccount(user, cloudProv);
    ...
    protected List<Plan> getPlan(Account user, String password) {
        if(UserModel.getInstance().authenUser(user.getUsername(), password)) {
            ...
            for (CloudAccount cloudAccount : cloudAccountList) {
                List<Plan> plans_tmp = vmProvider.getInstance().getPlanByCloudAccount(user, cloudAccount.getCloudProv().toString());
...
//        if(plan < plans.length){
            vmProvider.getInstance().changePlan(user, plan, CloudProvider.toEnum(cloudProv), ip);
            return true;
//        }
//        return false;
    }
